    ...
    form.addEventListener('submit', (event) => {
        ...
        try {
            ...
            feedback.innerHTML = `
                ...
                1. <em>Ficha_Alta_[Empresa].pdf</em><br>
                2. <em>Mandato_SEPA_[Empresa].pdf</em><br><br>
...
// --- PDF 2: MANDATO SEPA (REPLICA EXACTA) ---
function generateSepaPdf(data) {
    ...
    doc.setFontSize(14);
    doc.setFont('helvetica', 'bold');
    doc.text('Orden de domiciliación de adeudo directo SEPA', 105, 20, { align: 'center' });
    doc.setFontSize(10);
    doc.setFont('helvetica', 'italic');
    doc.text('SEPA Direct Debit Mandate', 105, 25, { align: 'center' });

    // 2. MARCO PRINCIPAL
    doc.setLineWidth(0.5);
    doc.rect(10, 35, 190, 230); // Marco exterior grande

    // --- SECCIÓN ACREEDOR (Parte Superior) ---
    ...
    doc.line(18, 35, 18, 95);

    // Datos del Acreedor (CV TOOLS - FIJOS)
    const startX = 22;
    let y = 42;
    ...
    doc.line(90, y+1, 195, y+1); // Línea subrayado
    ...
    // Línea separadora horizontal grande
    ...
    doc.text(doc.splitTextToSize(legalText, 180), 15, 100);
    doc.setFont('helvetica', 'italic');
    doc.text(doc.splitTextToSize(legalTextEn, 180), 15, 115);

    doc.line(10, 130, 200, 130); // Línea separadora

    // --- SECCIÓN DEUDOR (Parte Inferior) ---
    // Texto lateral izquierdo
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('A cumplimentar por el deudor', 13, 180, { angle: 90 });
    doc.text('To be completed by the debtor', 16, 180, { angle: 90 });
    
    doc.line(18, 130, 18, 265); // Línea vertical
    ...
    y += 5;
    doc.setFontSize(11);
    doc.text(data.nombre_empresa || '', startX, y); // DATO DEL FORMULARIO
    doc.line(startX, y+1, 195, y+1);
    ...
    doc.text(data.direccion || '', startX, y); // DATO DEL FORMULARIO
    doc.line(startX, y+1, 195, y+1);
    ...
    doc.text(direccionCompleta, startX, y); // DATO DEL FORMULARIO
    doc.line(startX, y+1, 195, y+1);
    ...
    y += 5;
    doc.setFontSize(11); doc.setFont('helvetica', 'normal');
    doc.text(data.pais || '', startX, y); // DATO DEL FORMULARIO
    doc.line(startX, y+1, 195, y+1);
    ...
    y += 5;
    doc.setFontSize(11);
    doc.text(data.swift || '', startX, y); // DATO DEL FORMULARIO
    doc.line(startX, y+1, 195, y+1);
    ...
    // Caja para el IBAN
    doc.setFontSize(12); doc.setFont('courier', 'bold'); // Courier para que parezca máquina
    doc.text(data.iban || '', startX, y); // DATO DEL FORMULARIO IBAN
    doc.line(startX, y+1, 195, y+1);
    
    doc.setFontSize(7); doc.setFont('helvetica', 'normal');
    doc.text('En España el IBAN consta de 24 posiciones comenzando siempre por ES', 60, y+5);
    ...
    doc.setFontSize(11); doc.setFont('helvetica', 'normal');
    // Rellenamos automáticamente la fecha y población si la tenemos
